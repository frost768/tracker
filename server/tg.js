        ...
        rl.question('Code: ', async (code) => {
            ...
            .then(async () => {
                if (!existsSync('./data/tgContacts.json')) {
                    ...
                    let ids = users = users.map(x=> x.id.toString());
                    contacts = contacts.users.filter(x => ids.includes(x.phone));
                    contacts.forEach(x => {
                        x.tag = users.find(y => y.id.toString() == x.phone).tag;
                    })
                    writeFileSync('./data/tgContacts.json', JSON.stringify(contacts, null, '\t'), { encoding: 'utf-8' })
...
tg.updates.on('updateShort', message => {
    ...
    if (update._ === 'updateUserStatus') {
        ...
        let tag = contacts.find(x => x.id == user_id).tag;
